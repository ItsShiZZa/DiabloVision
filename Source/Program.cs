...
namespace DiabloVision
{
    internal static class Program
    {
        ...
        public async static void D2RScanZoomValue()
        {
            ...
            {
                CurrentlyScanning = true;
                var scan = await MemLib.AoBScan(0, 0x7FFFFFFFFFFF, "00 00 06 00 00 00 ?? ?? ?? ?? ?? ?? ?? 43 ?? ?? ?? ?? ?? ?? ?? ?? 00 00 00 00 01 00 00 00 00 00 00 00 00 00 80 3F", false, true, false);
                long result = scan.FirstOrDefault();
                if (result != 0)
                {
                    CurrentlyScanning = false;
